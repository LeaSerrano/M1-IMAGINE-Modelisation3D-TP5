...
struct Grid {
    int resolution;
    std::map <unsigned int, Voxel> vectR(pow(resolution, 3));
...
struct Mesh {
    ...
    int getMinX {
        std::vector<Vec3> boite = calculBoiteEnglobante();
        //return boite[]
    }
    ...
    int getIndex(float x, float y, float z) {
        int i = 0, j = 0, k = 0;
        for (int elt = 0; elt < vertices.size(); elt++) {
            i = (int) ((x-minx)/grid.resolution/*(nx)*/)
        }
    }
